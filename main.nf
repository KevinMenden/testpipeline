...
========================================================================================
 nf-core/testpipeline Analysis Pipeline.
...
def helpMessage() {
    ...
    Usage:
n profile to use. Can use multiple (comma separated)
...
Channel.from(summary.collect { [it.key, it.value] })
    ...
    section_href: 'https://github.com/nf-core/testpipeline'
    plot_type: 'html'
    data: |
...
workflow.onComplete {
    ...
    if (!workflow.success) {
        subject = "[nf-core/testpipeline] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/testpipeline] Found multiple reports from process 'multiqc', will use only one"
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/testpipeline] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
    ...
    if (workflow.success) {
        log.info "-${c_purple}[nf-core/testpipeline]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
